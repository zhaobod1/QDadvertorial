...
else
{
  ...
  switch($action)
  {
   case "add":
	...
	if($rss!=NULL)
	{
		...
		echo "<script>alert('提交社区订单成功！');location.href='shequ.php';</script>";
	}
	else
	{
		echo "<script>alert('提交社区订单失败！');location.href='shequ.php';</script>";
	}
	break;
  ...
  case "pub":
   ...
   mysql_db_query($dbname,$sql);
   echo "<script>alert('发布操作成功！');location.href='shequ_list.php';</script>";
   break;
  ...
  case "rec":
   ...
   mysql_db_query($dbname,$sql);
   echo "<script>alert('收稿操作成功！');location.href='shequ_list.php';</script>";
   break;
  ...
  case "ref":	//拒稿，不进行资金结算
   ...
   if($rss!=NULL){
		...
		*/
		...
		mysql_db_query($dbname,$sql);
		echo "<script>alert('拒稿操作成功！');location.href='shequ_list.php';</script>";
   }
   else{
		...
		if($rss!=NULL){
			...
			echo "<script>alert('拒稿操作失败！');location.href='shequ_list.php';</script>";
		}
   }
   break;
  ...
  case "isdel":
	...
	$sql1="select * from shequ_info where ID=".$ID."";
	...
	$sql="update shequ_info set dyn=".$dyn." where ID=".$ID."";
	mysql_db_query($dbname,$sql);
	echo "<script>alert('删单操作成功！');location.href='shequ.php';</script>";
	break;
  ...
  case "del":
   ...
   echo "<script>alert('删除社区成功！');location.href='shequ.php';</script>";
   break;
  }
}
?>