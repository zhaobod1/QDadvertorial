...
else
{
  ...
  $sql="update media_info set nav_id=".$nav_id.",class_id=".$class_id.",small_id=".$small_id.",medianame='".$medianame."',diqu='".$diqu."',weburl='".$weburl."',anliurl='".$anliurl."',linkurl='".$linkurl."',xinwenyuan='".$xinwenyuan."',zhmh='".$zhmh."',fans=".$fans.",price=".$price.",dlprice=".$dlprice.",scprice=".$scprice.",content='".$content."',tj='".$tj."' where ID=".$ID."";
  mysql_db_query($dbname,$sql);
  //mysql_select_db($dbname);
  //$result=mysql_query($sql);
  echo "<script>alert('修改媒体资源成功！');location.href='wyx_list.php';</script>";
  break;
  ...
  case "isok":	//确认
   ...
   $yn=@$_REQUEST["yn"];
   if($yn==0){
	$sql="update wyx_info set title='".$title."',content='".$content."',flag=".$flag.",weburl='".$weburl."',reason='".$reason."' where ID=".$ID."";
   }
   else{
	$sql="update wyx_info set title='".$title."',content='".$content."',flag=".$flag.",weburl='".$weburl."',reason='".$reason."',yn=".$yn." where ID=".$ID."";
	
	$sqla="select * from wyx_info where ID='".$ID."'";
	$resulta=mysql_db_query($dbname,$sqla);
	$rsa=mysql_fetch_array($resulta);
	...
	elseif($flag==3){//拒稿
	   ...
	   if($rss!=NULL){
			...
			if($rss!=NULL){
				...
				echo "<script>alert('拒稿操作失败！');location.href='wyx.php';</script>";
			}
	   }
	}
   }
   ...
   //mysql_select_db($dbname);
   //$result=mysql_query($sql);
   echo "<script>alert('操作成功！');location.href='wyx.php';</script>";
   break;
  ...
  case "rec":	//收稿
   ...
   mysql_db_query($dbname,$sql);
   echo "<script>alert('收稿操作成功！');location.href='wyx.php';</script>";
   break;
  ...
  case "pub":	//发布
   ...
   mysql_db_query($dbname,$sql);
   echo "<script>alert('发布操作成功！');location.href='wyx.php';</script>";
   break;
  ...
  case "ref":	//拒稿，不进行资金结算
   ...
   if($rss!=NULL){
		...
		*/
		...
		mysql_db_query($dbname,$sql);
		echo "<script>alert('拒稿操作成功！');location.href='wyx.php';</script>";
   }
   else{
			$sqls="select * from admin where AdminUser='".$VipUser."'";
			$results=mysql_db_query($dbname,$sqls);
  ...
  case "isdel":	//审核删单申请，同意后执行退款处理
   ...
   if($rss!=NULL){
		...
		$sql="update wyx_info set reason='".$reason."',flag=".$flag." where ID=".$ID."";
		mysql_db_query($dbname,$sql);
		echo "<script>alert('拒稿操作成功！');location.href='wyx.php';</script>";
   }
   else{
