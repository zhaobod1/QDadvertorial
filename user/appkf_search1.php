	...
	switch($_GET["jg"])
		{
		 case "0":
			if($flag==1){
				$sql="select * from media_info where (dlprice between 0 and 40) and sh=1 and nav_id=54 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where (scprice between 0 and 40) and sh=1 and nav_id=54 order by ID asc";
			}
			else{
				$sql="select * from media_info where (price between 0 and 40) and sh=1 and nav_id=54 order by ID asc";
		   }
			break;
		 case "1":
			if($flag==1){
				$sql="select * from media_info where (dlprice between 41 and 70) and sh=1 and nav_id=54 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where (scprice between 41 and 70) and sh=1 and nav_id=54 order by ID asc";
			}
			else{
				$sql="select * from media_info where (price between 41 and 70) and sh=1 and nav_id=54 order by ID asc";
		   }
			break;
		 case "2":
			if($flag==1){
				$sql="select * from media_info where (dlprice between 71 and 120) and sh=1 and nav_id=54 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where (scprice between 71 and 120) and sh=1 and nav_id=54 order by ID asc";
			}
			else{
				$sql="select * from media_info where (price between 71 and 120) and sh=1 and nav_id=54 order by ID asc";
		   }
			break;
		 case "3":
			if($flag==1){
				$sql="select * from media_info where dlprice >120 and sh=1 and nav_id=54 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where scprice >120 and sh=1 and nav_id=54 order by ID asc";
			}
			else{
				$sql="select * from media_info where price >120 and sh=1 and nav_id=54 order by ID asc";
		   }
			break;
		 default:
...
elseif($_GET["zhmh"]<>""){
	...
	if($flag==2){
		$sql="select * from media_info where sh=1 and nav_id=54 and VipUser='".$VipUser."' ".$csql." ".$hsql." ".$dsql." ".$lsql." ".$xsql." ".$msql." ".$ssql."";
	}
	else{
