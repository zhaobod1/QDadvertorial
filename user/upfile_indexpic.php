...
?>
<?php
$setform=$_REQUEST["setform"];
$setpic=$_REQUEST["setpic"];
$setress=$_REQUEST["setress"];
...
'image/x-png'); 
$max_file_size=1024*1024*100;    //上传文件大小限制, 单位BYTE
$destination_folder="../upload/".$setress."/"; //上传文件路径
...
echo "<script>window.opener.document.".$setform.".".$setpic.".value='".$destination."';alert('上传成功');window.close();</script>";
}
}
?>
 <input name="upfile" type="file"   style="width:200;border:1 solid #9a9999; font-size:9pt; background-color:#ffffff" size="17">
<input type="submit" value="上传" style="width:30;border:1 solid #9a9999; font-size:9pt; background-color:#ffffff" size="17"><br>
</form> 
