...
if ($_COOKIE["fg"] > 3) {
	...
	switch ($action) {
		case "add":
			...
			$neturl = @$_REQUEST["neturl"];
			/*王庆路 根据judge值判断三种方式的哪一种 end*/
			$art_url_input = @$_REQUEST["art_url_input"];
			/*王庆路 上传文件方式*/
			if (strlen($art_url_input) > 10) {
				$path =  'huo15com->' . $art_url_input;
			}
			/*王庆路 上传文件方式 end*/
					...
					for ($i = 0; $i < count($mtmc); $i++) {
						...
						switch ($judge){
							case 1:
								...
								break;
							case 2:
								$sql = "insert into ruanwen_info (order_id,owner,mid,title,medianame,content,price,VipUser,data,flag) values ('" . $order_id . "','" . $owner . "'," . $mid . ",'" . $title . "','" . $medianame . "','" . $neturl . "'," . $price . ",'" . $VipUser . "','" . $data . "'," . $flag . ")";
								break;
							case 3:
								$sql = "insert into ruanwen_info (order_id,owner,mid,title,medianame,content,price,VipUser,data,flag) values ('" . $order_id . "','" . $owner . "'," . $mid . ",'" . $title . "','" . $medianame . "','" . $path . "'," . $price . ",'" . $VipUser . "','" . $data . "'," . $flag . ")";
								break;
							default:
								die("error");
								break;
		...
		case "edit":
			$title = @$_REQUEST["biaoti"];
			$content = @$_REQUEST["neirong"];
			/*王庆路	根据judge变量判断是三种选择方式的哪一种*/
			$neturl = @$_REQUEST["neturl"];
			$path = 'huo15com->'.@$_REQUEST['art_url_input'];
			$judge = @$_REQUEST["judge"];
			switch ($judge){
				case 1:
					...
					break;
				case 2:
					$sql = "update ruanwen_info set title='" . $title . "',content='" . $neturl . "' where ID=".$ID."";
					break;
				case 3:
					$sql = "update ruanwen_info set title='" . $title . "',content='" . $path . "' where ID=".$ID."";
					break;
				default:
					die("error");
					break;
			}
			/*王庆路	根据judge变量判断是三种选择方式的哪一种 end*/
			mysql_db_query($dbname, $sql);
