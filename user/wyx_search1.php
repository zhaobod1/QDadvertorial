...
if($_GET["fl"]<>""){
	$id=$_GET["fl"];
	$sql="select * from media_info where sh=1 and nav_id=47 and class_id=".$id." order by ID asc";
}
elseif($_GET["lj"]<>""){
	$id=$_GET["lj"];
	$sql="select * from media_info where sh=1 and nav_id=47 and linkurl='".$id."' order by ID asc";
}
elseif($_GET["xw"]<>""){
	$id=$_GET["xw"];
	$sql="select * from media_info where sh=1 and nav_id=47 and xinwenyuan='".$id."' order by ID asc";
}
	...
	switch($_GET["jg"])
		{
		 case "0":
			if($flag==1){
				$sql="select * from media_info where (dlprice between 0 and 40) and sh=1 and nav_id=47 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where (scprice between 0 and 40) and sh=1 and nav_id=47 order by ID asc";
			}
			else{
				$sql="select * from media_info where (price between 0 and 40) and sh=1 and nav_id=47 order by ID asc";
		   }
			break;
		 case "1":
			if($flag==1){
				$sql="select * from media_info where (dlprice between 41 and 70) and sh=1 and nav_id=47 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where (scprice between 41 and 70) and sh=1 and nav_id=47 order by ID asc";
			}
			else{
				$sql="select * from media_info where (price between 41 and 70) and sh=1 and nav_id=47 order by ID asc";
		   }
			break;
		 case "2":
			if($flag==1){
				$sql="select * from media_info where (dlprice between 71 and 120) and sh=1 and nav_id=47 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where (scprice between 71 and 120) and sh=1 and nav_id=47 order by ID asc";
			}
			else{
				$sql="select * from media_info where (price between 71 and 120) and sh=1 and nav_id=47 order by ID asc";
		   }
			break;
		 case "3":
			if($flag==1){
				$sql="select * from media_info where dlprice >120 and sh=1 and nav_id=47 order by ID asc";
			}
			elseif($flag==2){
				$sql="select * from media_info where scprice >120 and sh=1 and nav_id=47 order by ID asc";
			}
			else{
				$sql="select * from media_info where price >120 and sh=1 and nav_id=47 order by ID asc";
		   }
			break;
		 default:
			$sql="select * from media_info where sh=1 and nav_id=47 order by ID asc";
		}
}
elseif($_GET["dq"]<>""){
	$id=$_GET["dq"];
	$sql="select * from media_info where sh=1 and nav_id=47 and diqu='".$id."' order by ID asc";
}
elseif($_GET["zhmh"]<>""){
	$id=$_GET["zhmh"];
	$sql="select * from media_info where sh=1 and nav_id=47 and zhmh='".$id."' order by ID asc";
}
elseif($_GET["tejia"]<>""){
	$id=$_GET["tejia"];
	$sql="select * from media_info where sh=1 and nav_id=47 and small_id='".$id."' order by ID asc";
}
else{
	$mc=$_GET["mc"];
	$cid=$_GET["cid"];
	...
	if($flag==2){
		$sql="select * from media_info where sh=1 and nav_id=47 and VipUser='".$VipUser."' ".$csql." ".$hsql." ".$dsql." ".$lsql." ".$xsql." ".$msql." ".$ssql."";
	}
	else{
		$sql="select * from media_info where sh=1 and nav_id=47 ".$csql." ".$hsql." ".$dsql." ".$lsql." ".$xsql." ".$msql." ".$ssql."";
	}
}
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC" id="searchtr" style="font-size:13px">
  ...
  <tr>
	...
	<th width="80" bgcolor="#CDD3EB"><div align="center"><strong>粉丝数量</strong></div></th>
<?php if($flag==1){?>
...
<?php
	...
	<td width="100" ><div align="center"><?php echo $rs["fans"];?></div></td>
