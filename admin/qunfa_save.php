...
else
{
  ...
  //echo "<script>alert('群发：".$VipUser."　标题：".$title."　内容：".$content."　金额：".$price."');</script>";
	...
	if($rss!=NULL)
	{
		...
		if(count($mtmc)>1){
			for($i=0;$i<count($mtmc);$i++){
				...
				$sql="insert into qunfa_info (order_id,owner,mid,title,medianame,content,price,VipUser,data,flag) values ('".$order_id."','".$owner."',".$mid.",'".$title."','".$medianame."','".$content."',".$price.",'".$VipUser."','".$data."',".$flag.")";
		...
		echo "<script>alert('提交群发订单成功！');location.href='qunfa.php';</script>";
	}
	else
	{
		echo "<script>alert('提交群发订单失败！');location.href='qunfa.php';</script>";
	}
	break;
 
  case "edit":	//编辑
   
   $sql="update qunfa_info set title='".$title."',content='".$content."' where ID=".$ID."";
   mysql_db_query($dbname,$sql);
   //mysql_select_db($dbname);
   //$result=mysql_query($sql);
   echo "<script>alert('成功保存修改！');location.href='qunfa.php';</script>";
   break;
  ...
  //mysql_select_db($dbname);
  //$result=mysql_query($sql);
  echo "<script>alert('修改媒体资源成功！');location.href='qunfa_list.php';</script>";
  break;
  ...
  case "isok":	//确认
   ...
   $yn=@$_REQUEST["yn"];
   if($yn==0){
	$sql="update qunfa_info set title='".$title."',content='".$content."',flag=".$flag.",weburl='".$weburl."',reason='".$reason."' where ID=".$ID."";
   }
   else{
	$sql="update qunfa_info set title='".$title."',content='".$content."',flag=".$flag.",weburl='".$weburl."',reason='".$reason."',yn=".$yn." where ID=".$ID."";
	
	$sqla="select * from qunfa_info where ID='".$ID."'";
	$resulta=mysql_db_query($dbname,$sqla);
	$rsa=mysql_fetch_array($resulta);
	...
	elseif($flag==3){//拒稿
	   ...
	   if($rss!=NULL){
			...
			if($rss!=NULL){
				...
				echo "<script>alert('拒稿操作失败！');location.href='qunfa.php';</script>";
			}
	   }
	}
   }
   ...
   //mysql_select_db($dbname);
   //$result=mysql_query($sql);
   echo "<script>alert('操作成功！');location.href='qunfa.php';</script>";
   break;
   
  case "rec":	//收稿
  
   $flag=@$_REQUEST["flag"];
   $sql="update qunfa_info set flag=".$flag." where ID=".$ID."";
   mysql_db_query($dbname,$sql);
   echo "<script>alert('收稿操作成功！');location.href='qunfa.php';</script>";
   break;
  ...
  case "pub":	//发布
   ...
   $weburl=@$_REQUEST["weburl"];
   if($flag==2){
	$sql="update qunfa_info set flag=".$flag.",weburl='".$weburl."',fbtime='".$data."' where ID=".$ID."";
   }
   else{
	$sql="update qunfa_info set flag=".$flag.",weburl='"."',fbtime='"."' where ID=".$ID."";
   }
   mysql_db_query($dbname,$sql);
   echo "<script>alert('发布操作成功！');location.href='qunfa.php';</script>";
   break;
  ...
  case "ref":	//拒稿，不进行资金结算
   ...
   if($rss!=NULL){
		...
		*/
		...
		$sql="update qunfa_info set reason='".$reason."',flag=".$flag.",fbtime='".$data."' where ID=".$ID."";
		mysql_db_query($dbname,$sql);
		echo "<script>alert('拒稿操作成功！');location.href='qunfa.php';</script>";
   }
   else{
			$sqls="select * from admin where AdminUser='".$VipUser."'";
			$results=mysql_db_query($dbname,$sqls);
  ...
  case "isdel":	//审核删单申请，同意后执行退款处理
   ...
   if($rss!=NULL){
		...
		$sql="update qunfa_info set reason='".$reason."',flag=".$flag." where ID=".$ID."";
		mysql_db_query($dbname,$sql);
		echo "<script>alert('拒稿操作成功！');location.href='qunfa.php';</script>";
   }
   else{
  ...
  case "del":
   ...
   {
    $sql2="delete from qunfa_info where ID=".$it."";
    mysql_db_query($dbname,$sql2);
    //mysql_select_db($dbname);
    //$result2=mysql_query($sql2);
   }
   echo "<script>alert('删除群发成功！');location.href='qunfa.php';</script>";
   break;
  }
}
?>